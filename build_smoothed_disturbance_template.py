...
def circular_moving_average(values, window_size):
    ...
    这里使用“环形（circular）滑动平均”，而不是普通滑动平均，
    因为 phase 模板是周期性的：最后一个 bin 和第一个 bin 在物理上首尾相接。
...
def build_smoothed_template(template, window_size):
    ...
    smoothed_template = {
        ...
        # 版本 B 的核心模板
        "torso_linear_acceleration_template": acc_raw,
...
def main():
    parser = argparse.ArgumentParser(description="基于 disturbance_template.npz 生成版本 B 半平滑模板")
    parser.add_argument(
        "--input",
        type=str,
    ...
    try:
        ...
        smoothed_template = build_smoothed_template(template, args.window_size)
