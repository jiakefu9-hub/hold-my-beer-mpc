...
| 15–18 秒 | 前进速度为零，继续用航向保持纠正相对 H0 的方向；双臂继续保持 |
...
- 正常时约每 50 ms 刷新速度，单次请求 duration 最长 0.2 秒；临近第 15、18 秒都会缩短
  请求时长。第 15 秒把前进速度置零，但航向修正继续到第 18 秒；第 18 秒先要求收到
  `vx=0、wz=0` 的成功回复，再开始手臂退权。
